    ...
    def goal_portfolio(self, positions, algo="", algo_param={}, userdata=""):
        ...
        if type(positions) is pd.core.frame.DataFrame :
            ...
            for i in xrange(0, len(positions)):
                tmp.append ({'security': positions.index[i],'ref_price': float(positions['ref_price'][i]), "size" : int(positions['size'][i])})
            positions = tmp
    ...
    def basket_order(self, orders, algo="", algo_param={}, userdata=""):
        ...
        if type(orders) is pd.core.frame.DataFrame :
            ...
            for i in xrange(0, len(orders)):
                tmp.append ({'security': orders.index[i],'ref_price': float(orders['ref_price'][i]), "inc_size" : int(orders['inc_size'][i])})
            orders = tmp
