...
class DataView(object):
    ...
    def __init__(self):
        ...
        self.meta_data_list = [
            'start_date', 'end_date', 'extended_start_date_d',
            'extended_start_date_q', 'freq', 'fields', 'symbol', 'universe',
            'benchmark', 'custom_daily_fields', 'custom_quarterly_fields'
        ]
        ...
        market_bar_list = {
            'open', 'high', 'low', 'close', 'volume', 'turnover', 'vwap', 'oi'
        }
        market_tick_list = {
            'volume', 'oi', 'askprice1', 'askprice2', 'askprice3', 'askprice4',
            'askprice5', 'bidprice1', 'bidprice1', 'bidprice1', 'bidprice1',
            'bidprice1', 'askvolume1', 'askvolume2', 'askvolume3',
            'askvolume4', 'askvolume5', 'bidvolume1', 'bidvolume2',
    ...
    def data_benchmark(self, df_new):
        if self.data_d is not None and df_new.shape[0] != self.data_d.shape[0]:
            raise ValueError(
                "You must provide a DataFrame with the same shape of data_benchmark."
            )
    ...
    def dates(self):
        ...
        if self.data_d is not None:
            ...
            res = self.data_api.query_trade_dates(self.extended_start_date_d,
                                                  self.end_date)
        else:
            raise ValueError(
                "Cannot get dates array when neither of data and data_api exists."
            )
    ...
    def _is_predefined_field(self, field_name):
        ...
        """
        return self._is_quarter_field(field_name) or self._is_daily_field(
            field_name)
    ...
    def _get_fields(self, field_type, fields, complement=False, append=False):
        ...
        """
        pool_map = {
            'market_daily': self.market_daily_fields,
        ...
        pool_map['daily'] = set.union(pool_map['market_daily'],
                                      pool_map['ref_daily'], pool_map['group'],
                                      self.custom_daily_fields)
        pool_map['quarterly'] = set.union(
            pool_map['income'], pool_map['balance_sheet'],
            pool_map['cash_flow'], pool_map['fin_indicator'],
            self.custom_quarterly_fields)
        ...
        if append:
            ...
            elif (field_type == 'income' or field_type == 'balance_sheet'
    ...
    def init_from_config(self, props, data_api):
        ...
        self.extended_start_date_d = jutil.shift(
            self.start_date, n_weeks=-8)  # query more data
        ...
        if fields:
            ...
            self.fields = [
                field for field in fields if self._is_predefined_field(field)
            ]
            if len(self.fields) < len(fields):
                print("Field name [{}] not valid, ignore.".format(
                    set.difference(set(fields), set(self.fields))))

        # append additional fields
        if self.all_price:
            self.fields.extend([
                'open_adj', 'high_adj', 'low_adj', 'close_adj', 'open', 'high',
                'low', 'close', 'vwap', 'vwap_adj'
            ])
        ...
        if universe:
            ...
            for univ in univ_list:
                symbols_list.extend(
                    data_api.query_index_member(
                        univ, self.extended_start_date_d, self.end_date))
            ...
            if self.universe:
                if len(self.universe) > 1:
                    print("More than one universe are used: {}, "
                          ...
                          "please specify benchmark in configs.".format(
                              repr(self.universe), self.universe[0]))
    ...
    def distributed_query(self,
        ...
        if n_symbols * n_days > limit:
            ...
            while pos2 < n_days:
                print(pos2)
                df, msg = getattr(self.data_api, query_func_name)(
                    symbol=symbol,
    ...
    def _prepare_daily_quarterly(self, fields):
        ...
        if daily_list:
            daily_list_pivot = [
                pivot_and_sort(df, self.TRADE_DATE_FIELD_NAME)
                for df in daily_list
            ]
            multi_daily = self._merge_data(daily_list_pivot,
                                           self.TRADE_DATE_FIELD_NAME)
            # use self.dates as index because original data have weekends
            multi_daily = self._fill_missing_idx_col(
                multi_daily, index=self.dates, symbols=self.symbol)
            print("Query data - daily fields prepared.")
        if quarterly_list:
            quarterly_list_pivot = [
                pivot_and_sort(df, self.REPORT_DATE_FIELD_NAME)
                for df in quarterly_list
            ]
            multi_quarterly = self._merge_data(quarterly_list_pivot,
                                               self.REPORT_DATE_FIELD_NAME)
            multi_quarterly = self._fill_missing_idx_col(
                multi_quarterly, index=None, symbols=self.symbol)
    ...
    def _query_data(self, symbol, fields):
        ...
        if self.freq == 1:
            ...
            fields_market_daily = self._get_fields(
                'market_daily', fields, append=True)
            if fields_market_daily:
                print("NOTE: price adjust method is [{:s} adjust]".format(
                    self.adjust_mode))
                # no adjust prices and other market daily fields
                df_daily, msg1 = self.distributed_query(
                    'daily',
                    symbol_str,
                ...
                if self.all_price:
                    ...
                    df_daily_adjust, msg1 = self.distributed_query(
                        'daily',
                        symbol_str,
                        start_date=self.extended_start_date_d,
                        end_date=self.end_date,
            ...
            fields_ref_daily = self._get_fields(
                'ref_daily', fields, append=True)
            if fields_ref_daily:
                df_ref_daily, msg2 = self.distributed_query(
            ...
            if fields_income:
                df_income, msg3 = self.data_api.query_lb_fin_stat(
                    'income',
                    symbol_str,
            ...
            fields_balance = self._get_fields(
                'balance_sheet', fields, append=True)
            if fields_balance:
            ...
            if fields_cf:
                df_cf, msg3 = self.data_api.query_lb_fin_stat(
                    'cash_flow',
                    symbol_str,
                    self.extended_start_date_q,
                    self.end_date,
            ...
            fields_fin_ind = self._get_fields(
                'fin_indicator', fields, append=True)
            if fields_fin_ind:
    ...
    def _fill_missing_idx_col(self, df, index=None, symbols=None):
        ...
        if len(fields) * len(self.symbol) != len(
                df.columns) or len(index) != len(df.index):
            cols_multi = pd.MultiIndex.from_product(
                [symbols, fields], names=['symbol', 'field'])
            cols_multi = cols_multi.sort_values()
            df_final = pd.DataFrame(
                index=index, columns=cols_multi, data=np.nan)
            ...
            col_diff = sorted(
                set(df_final.columns.levels[0].values) -
                set(df.columns.levels[0].values))
    ...
    def _align_and_merge_q_into_d(self):
        ...
        if data_d is not None and data_q is not None:
            df_ref_ann = data_q.loc[:,
                                    pd.IndexSlice[:, self.
                                                  ANN_DATE_FIELD_NAME]].copy()
            df_ref_ann.columns = df_ref_ann.columns.droplevel(level='field')

            dic_expanded = dict()
            for field_name, df in data_q.groupby(
                    level=1, axis=1):  # by column multiindex fields
            ...
            data_d_merge = self._merge_data(
                [data_d, df_quarterly_expanded],
    ...
    def _prepare_adj_factor(self):
        ...
        if mask_stocks.sum() > 0:
            ...
            df_adj = self.data_api.query_adj_factor_daily(
                symbol_str,
                start_date=self.extended_start_date_d,
                end_date=self.end_date,
                div=False)
        else:
            dt_idx = self.data_api.query_trade_dates(
                start_date=self.extended_start_date_d,
                end_date=self.end_date,
            )
            df_adj = pd.DataFrame(
                index=dt_idx, columns=self.data_inst.index).fillna(1)
    ...
    def _prepare_comp_info(self):
        ...
        for univ in self.universe:
            df = self.data_api.query_index_member_daily(
                univ, self.extended_start_date_d, self.end_date)
            res[univ] = df
        df_res = pd.concat(res, axis=0)
        df = df_res.groupby(
            by='trade_date').apply(lambda df: df.any(axis=0)).astype(float)
        ...
        df_weights = self.data_api.query_index_weights_daily(
            self.universe[0], self.extended_start_date_d, self.end_date)
    ...
    def _prepare_inst_info(self):
        res = self.data_api.query_inst_info(
    ...
    def _prepare_group(self, group_fields):
        ...
        for field in group_fields:
            ...
            df = self.data_api.query_industry_daily(
                symbol=','.join(self.symbol),
    ...
    def _prepare_benchmark(self):
        df_bench, msg = self.data_api.daily(
            self.benchmark,
            start_date=self.extended_start_date_d,
            end_date=self.end_date,
            adjust_mode=self.adjust_mode,
    ...
    def _add_field(self, field_name, is_quarterly=None):
        ...
        if not self._is_predefined_field(field_name):
            if is_quarterly is None:
                raise ValueError(
                    "Field [{:s}] is not a predefined field, but no frequency information is provided."
                )
            if is_quarterly:
    ...
    def add_field(self, field_name, data_api=None):
        ...
        if self._is_daily_field(field_name):
            if self.data_d is None:
                raise ValueError(
                    "Please prepare [{:s}] first.".format(field_name))
        ...
        self.append_df(
            merge, field_name, is_quarterly=is_quarterly
    ...
    def add_formula(self,
        ...
        if within_index:
            ...
            df_eval = parser.evaluate(
                var_df_dic, ann_dts=df_ann, trade_dts=self.dates)
    ...
    def append_df(self, df, field_name, is_quarterly=False):
        ...
        if len(df.columns) < len(exist_symbols):
            df2 = pd.DataFrame(
                index=df.index, columns=exist_symbols, data=np.nan)
            df2.update(df)
            df = df2
        ...
        the_data = pd.merge(
            the_data, df, left_index=True, right_index=True, how='left')
    ...
    def remove_field(self, field_names):
        ...
        for field_name in field_names:
            ...
            if field_name not in self.fields:
                print("Field name [{:s}] does not exist. Stop remove_field.".
                      format(field_name))
                return
            ...
            if self._is_daily_field(field_name):
                ...
                print("Field name [{}] is a pre-defined field, ignore.".format(
                    field_name))
                return
    ...
    def get(self, symbol="", start_date=0, end_date=0, fields=""):
        ...
        res = self.data_d.loc[pd.IndexSlice[start_date:end_date],
                              pd.IndexSlice[symbol, fields]]
        return res
    ...
    def get_snapshot(self, snapshot_date, symbol="", fields=""):
        ...
        res = self.get(
            ...
            fields=fields)
        if res is None:
            print("No data. for date={}, fields={}, symbol={}".format(
                snapshot_date, fields, symbol))
            return
    ...
    def get_symbol(self, symbol, start_date=0, end_date=0, fields=""):
        ...
        if res is None:
            raise ValueError(
                "No data. for "
                "start_date={}, end_date={}, field={}, symbol={}".format(
                    start_date, end_date, fields, symbol))
    ...
    def get_ts_quarter(self, field, symbol="", start_date=0, end_date=0):
        ...
        df_ref_quarterly.columns = df_ref_quarterly.columns.droplevel(
            level='field')
    ...
    def get_ts(self,
        ...
        """
        res = self.get(
            symbol, start_date=start_date, end_date=end_date, fields=field)
        if res is None:
            print(
                "No data. for start_date={}, end_date={}, field={}, symbol={}".
                format(start_date, end_date, field, symbol))
            raise ValueError
            return
    ...
    def _process_data(self, large_memory=False):
        ...
        if t is None or len(t.columns) == 0:
            ...
            preclose = self.get_ts(
                'close', start_date=before_first_day).shift(1)
    ...
    def load_dataview(self, folder_path='.', large_memory=True):
        ...
        if not (os.path.exists(path_meta_data) and os.path.exists(path_data)):
            raise IOError(
                "There is no data file under directory {}".format(folder_path))
    ...
    def save_dataview(self, folder_path):
        ...
        data_to_store = {
            k: v
            for k, v in data_to_store.items() if v is not None
        }
        meta_data_to_store = {
            key: self.__dict__[key]
            for key in self.meta_data_list
        }
...
class EventDataView(object):
    ...
    def __init__(self):
        ...
        self.meta_data_list = [
            'start_date', 'end_date', 'extended_start_date_d', 'freq',
            'fields', 'symbol', 'universe', 'benchmark', 'custom_daily_fields'
        ]
        self.adjust_mode = 'post'
    ...
    def _add_symbol(self, symbol_name):
        if symbol_name in self.symbol:
            print("symbol [{:s}] already exists, add_symbol failed.".format(
                symbol_name))
            return
    ...
    def append_df_symbol(self, df, symbol_name):
        ...
        if len(set(exist_fields) - set(df.columns)):
            ...
            df2 = pd.DataFrame(
                index=df.index, columns=exist_fields, data=np.nan)
            df2.update(df)
            df = df2
    ...
    def get_ts(self, field, symbol="", start_date=0, end_date=0):
        ...
        if res is None:
            raise ValueError(
                "No data. for "
                "start_date={}, end_date={}, field={}, symbol={}".format(
                    start_date, end_date, field, symbol))
